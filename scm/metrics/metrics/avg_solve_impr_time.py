...
@metric.element(name="Время на дорешивание", id="avg_solve_improvement_time_metric")
class AvgSolveImpovementTimeMetric:
    description = """Среднее время, которое ученики тратят на дорешивание задачи"""

    # jackie_chan_confused.jpg
    # Author: Sergey
    @staticmethod
    def calc(data, config):
        merge_user_element_and_log = pd.merge(data.user_element_progress, data.solution_log, how="inner", left_on='id', right_on='element_progress_id')     #обьединение логов попыток сдать задачу и самих задач
        ...
        dt = pd.merge(dt, data.course_element.loc[data.course_element['element_type'] == 'task'],how = "inner", left_on = ['course_module_id','course_element_id'], right_on= ['module_id','element_id'])[['course_id','course_module_id','course_element_id','id', 'value', 'sample']]
